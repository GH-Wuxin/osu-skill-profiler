...
Frontiers are found by scanning every observed difficulty threshold.  Partial support follows a versioned policy exponent at every candidate
threshold.  The exponent therefore participates in winner selection instead of
being applied after a linear winner has already been chosen.
"""
...
SCHEMA_VERSION = "axis_support_frontier_v02"
...
class SupportPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        for field_name, value in zip(mix_names, mixes):
            object.__setattr__(self, field_name, value)
        partial_support_exponent = _finite_float(self.partial_support_exponent)
        if partial_support_exponent is None or partial_support_exponent < 1.0:
            raise ValueError(
                "SupportPolicy.partial_support_exponent must be finite and at least 1"
            )
        object.__setattr__(self, "partial_support_exponent", partial_support_exponent)
...
def _frontier_star(
    ...
    """Apply powered partial support before comparing candidate thresholds."""

    ratio = min(1.0, max(0.0, support) / target)
    return threshold * ratio**exponent
...
def _empty_result(policy: SupportPolicy, evidence_confidence: object, ignored: int) -> dict[str, Any]:
    ...
    return {
        ...
        "diagnostics": {
            ...
            "threshold_scan": "ALL_OBSERVED_POWERED_PARTIAL_SUPPORT",
            "partial_support_exponent": policy.partial_support_exponent,
            "partial_support_mapping": "THRESHOLD_TIMES_SUPPORT_RATIO_POWER",
        },
    }
            ...
            "establishment": {
                **common,
                "frontier_star": _frontier_star(
                    ...
                    policy.frontier_support_target,
                    policy.partial_support_exponent,
                ),
